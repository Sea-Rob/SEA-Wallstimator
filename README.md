...
- **Live capture coaching** (issue #5): while recording, cheap per-frame checks run in `PanCore::push_frame` alongside keyframe selection and the page shows ONE prominent coaching line (highest-priority check wins: lost marker > too fast > exposure > rotation). The checks reuse what the recorder already computes — thumbnail shift → motion speed ("slow down", with hysteresis), thumbnail luma histogram → exposure ("too dark — turn on a light" / blown out), and Reference Marker visibility via detection on a half-scale plane every 5th frame ("start with Marker A in view", "no marker seen for a while"). Rotation-dominant panning (standing in place and swiveling — refused post-capture by the closure plausibility guard, issue #19) is coached away live via a flow-curvature heuristic: an in-place rotation moves frame edges faster than the centre (the depth-independent K·R·K⁻¹ profile), which uniform wall-parallel translation lacks; three seeded NCC probes on a quarter-scale plane accumulate that signature. Honest limits: it needs texture across the frame, takes ~1 s of sustained swivel to trip, and misses a small constant yaw lead while genuinely walking. A recording that stops without BOTH markers ever sighted — or that was mostly blurred/untrackable — gets an immediate retake prompt with the reason instead of processing. Checks are deterministic core functions tested on synthetic sequences (`crates/geometry-core/tests/live_coaching.rs`).
