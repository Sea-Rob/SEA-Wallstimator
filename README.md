...
- `web/` — capture page (plain JS, no framework), `print-scale.js` (pure correction/plausibility math), `camera.js` (pure main-lens / zoom-lock heuristics), `session.js` (in-memory session state), and a tiny dev server that sends the COOP/COEP headers.
